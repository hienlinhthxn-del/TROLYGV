...
const StudentPractice: React.FC<StudentPracticeProps> = ({ subject, grade, questions, onExit }) => {
  ...
  const handleSelectOption = (optionIdx: number) => {
    ...
    setAnswers({ ...answers, [currentQuestion.id]: labels[optionIdx] });
  };
         ...
         <div className="w-full max-w-3xl space-y-8 mb-24">
            ...
            <div className="flex items-center justify-between px-4">
               <button 
                 ...
                 className="flex items-center space-x-3 text-slate-400 hover:text-indigo-600 font-black text-[10px] uppercase tracking-widest disabled:opacity-20 transition-all"
               >
               ...
               <div className="flex space-x-2">
                  {questions.map((_, i) => (
                    <button 
                      ...
                      className={`w-2 h-2 rounded-full transition-all ${i === currentIdx ? 'w-8 bg-indigo-600' : answers[questions[i].id] ? 'bg-emerald-400' : 'bg-slate-200'}`}
                    />
                  ))}
               </div>
               <button 
